...
(function (window, undefined) {
  ...
  function _getText(el) {
    ...
    // }
    // else {
     // replace img tag to markdown tag
     // it will use jquery
     // if ($){
     //    editor.img_count = editor.img_count || 0;
     //    var copy_el = $.extend({}, el);
     //    for (var i = 0; i < imgs.length; i++) {
     //        editor.img_count ++;
     //        var now_img = $(imgs[i]);
     //        var src = now_img.attr('src');
     //        var markdown_s = '![figure ' + editor.img_count + '](' + src + ' \'figure ' + editor.img_count + '\')'
     //        now_img.replaceWith(markdown_s);
     //    };
     // }
    ...
    // Now we can clean the HTML, but replace html img to markdown 
    theText = theText.replace(/<(?:.|\n)*?>/gm, function(s){
      if ( s.length < 10000 && s.indexOf('<img') === 0){
  
          s =  s.slice(s.indexOf('src="') + 5);
          var src_e = s.indexOf('"')
          var src = s.slice(0, src_e);
          var markdown_s = '![picture ](' + src + ' \'picture \')';
          return markdown_s;
      }
      return '';
    });
  ...
  function _setText(el, content) {
    ...
    content = content.replace(/^ /, '&nbsp;')
    // replace markdown img to html img
    content = content.replace(/!\[picture(?:.|\s)*?picture\s'\)/gm, function(s){
        var src_s = s.indexOf('(');
        var src_e = s.indexOf(' \'picture')
        var src = s.slice(src_s + 1, src_e);
        var markdown_s = '<img src="' + src + '">';
        return markdown_s;
      });
  ...
  function EpicEditor(options) {
    ...
    var self = this
      ...
      , defaultStorage
      , autogrowDefaults = { minHeight: window.innerHeight
        , maxHeight: false
        , scroll: true
        };
    // alert(autogrowDefaults.minHeight);
